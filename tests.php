...
require_once dirname( __FILE__ ) . '/lib/class-amp-dom-utils.php';
require_once dirname( __FILE__ ) . '/lib/class-amp-string-utils.php';
...
class AMPSanitizerTests {
	...
	public function run_test( $n ) {
		...
		for ( $i = 0; $i < $n; $i++ ) {
			...
			$sanitizer = new AMP_Blacklist_Sanitizer( $dom, array() );
			$sanitizer->sanitize();
			AMP_DOM_Utils::get_content_from_dom( $dom );

			$stop = $this->stop_timer();

			$this->add_result( 'AMP_Blacklist_Sanitizer', 'Results', $this->get_elapsed_time( $start, $stop ) );
		}

		// Style with processing
		for ( $i = 0; $i < $n; $i++ ) {
			$start = $this->start_timer();
	...
	public function get_test_content() {
		ob_start(); ?>
